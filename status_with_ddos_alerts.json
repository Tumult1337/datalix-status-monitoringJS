...
const data = [{
    ...
    alerts: "", // ddos alert webhook
    lastState: "", // placeholder, will get filled by the script
    ipv4: null,
}];

const userAgent = "XMLHttpRequest";
var rqDelay = data.length + 2 || 2; // default 2 sec per service 2 sec delay (ratelimit is 30rq/60sec)
...
function sendEmbed(title, details, index, color, webhook) {
    const out = {
            'author': {
            ...
            'title': title || "Server Status Update",
            'url': "https://datalix.de/cp/service/" + data[index].service,
            'color': color,
            fields: details,
            timestamp: new Date(),
    ...
    xhr.onload = function() {
        console.log("Status update for " + data[index].name || "Service " + data[index].service + " | DDoS or Status")
    }
...
let mostOccurringMethod = {};
let lastSentCreationTime = "";
let mostOccurringMethodSent = false;

function ddoslogs(index) {
    xmlHttp = new XMLHttpRequest();
    response = "";
    startTime = Date.now()
    xmlHttp.open("GET", getURL(index, "incidents"), true);
    xmlHttp.setRequestHeader('User-Agent', userAgent);
    xmlHttp.send();
    xmlHttp.onload = function() {
        try {
            ...
            if (xmlHttp.status != 200) {
                console.log("HTTP error0: " + xmlHttp.status);
            ...
            if (response && response.data && response.data.length > 0) {
                // Include the most occurring attack method in the embed only if not already sent
                const latestCreationTime = response.data[0].created_on;
                if (lastSentCreationTime !== latestCreationTime) {
                    // Convert embedDetails to a string format
                    const embedDetails = [
                        `**IP Address:** ${response.data[0].ip}`,
                        `**Mbps:** ${response.data[0].mbps}`,
                        `**PPS:** ${response.data[0].pps}`,
                        `**Mode:** ${response.data[0].mode}`,
                        `**Attack Method:** ${response.data[0].method}`,
                        `**Most Occurring Attack Method:** ${findMostOccurringMethod(response.data)}`,
                        `**Created On:** ${latestCreationTime}`,
                    ].join("\n");

                    // Update most occurring attack method
                    const latestAttackMethod = response.data[0].method;
                    if (!mostOccurringMethod[latestAttackMethod]) {
                        mostOccurringMethod[latestAttackMethod] = 1;
                    } else {
                        mostOccurringMethod[latestAttackMethod]++;
                    }

                    // Send Discord embed
                    sendEmbed("DDoS Attack alert", [{name: "Service: " + data[index].name, value: embedDetails}], index, 0x00FF69, data[index].alerts);
                    lastSentCreationTime = latestCreationTime;
                    mostOccurringMethodSent = true;
                }
            }
		...
		xmlHttp.onload = function() {
			try {
				...
				if (xmlHttp.status == 403) {
					...
					sendEmbed("Server Status Update", embedData, index, apiStateDesc["deletedService"].color, data[index].webhook);
				...
				sendEmbed("Server Status Update", details, index, state.color, data[index].webhook)
			} catch (rushEsukabljat) {
				console.log(rushEsukabljat);
			}
		}
...
// Update the function to find and log the most occurring attack method
function findMostOccurringMethod(apiData) {
    let methodCounts = {};
  
    // Count occurrences of each attack method
    ...
    // Find the most occurring attack method
    let mostOccurringMethod = "";
    let mostOccurringCount = 0;
  
    for (const method in methodCounts) {
      if (methodCounts[method] > mostOccurringCount) {
        mostOccurringCount = methodCounts[method];
        mostOccurringMethod = method;
      }
    }
  
    console.log("Most Occurring Attack Method:", mostOccurringMethod);
  
    return mostOccurringMethod;
  }


// Add a function to find and log the most occurring attack method
function main() {
    try {
        setTimeout(function() {
            main();
